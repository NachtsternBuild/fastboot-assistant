...
void buttonfs_clicked(GtkWidget *widget, gpointer data)
{
    flashsystemfunction();
}

// UI for flashsystem
void flashsystem(int argc, char *argv[])
{
    // Initialize GTK
    gtk_init(&argc, &argv);
    ...
    // Create a button
    GtkWidget *button = gtk_button_new_with_label("Flash System-Image");
    g_signal_connect(button, "clicked", G_CALLBACK(buttonfs_clicked), entry);
