...
void rm_rom_install()
{
    ...
    char command[550];
    char rom_install_rm_file[256];
    char main_path_rm[512];
...
{
    LOGD("remove_backups");
    char backup_rm_file[256];
    char two_commands[1024];
...
void remove_old(GtkWidget *widget, gpointer stack) 
{
    ...
    GtkWidget *btn_back = create_button_icon_position("pan-start-symbolic", labels[3], G_CALLBACK(config_project_GUI), stack, GTK_ALIGN_CENTER);
