...
void reboot(int argc, char *argv[]) 
{
    ...
    // Create a button
    GtkWidget *button_reboot_system = gtk_button_new_with_label("Neustarten");
    g_signal_connect(button_reboot_system, "clicked", G_CALLBACK(reboot_system), NULL);

    // Pack the button into the box
    gtk_box_pack_start(GTK_BOX(vbox), button_reboot_system, TRUE, TRUE, 0);
