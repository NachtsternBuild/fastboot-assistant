...
void on_file_set_sys(GtkFileChooser *chooser, gpointer data)
{
    // get filename from userchoice
    gchar *filename = gtk_file_chooser_get_filename(chooser);
    const char *newname = "system.img";
...
// UI for rename file
void renamefilesys(int argc, char *argv[])
{
    gtk_init(&argc, &argv);
    ...
    g_signal_connect(window, "file-set", G_CALLBACK(on_file_set_sys), NULL);
