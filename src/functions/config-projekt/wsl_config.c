...
static void config_adb(GtkWidget *widget) 
{
    for (int i = 0; i < 2; ++i) 
    {
        for (int j = 0; j < 2; ++j) 
        {
            ...
            command_pkexec_spinner(widget, full_command, title, text);
        }
    }
}

// main function wsl_config
void wsl_config(GtkWidget *widget, gpointer stack) 
{    
    int found = 0;
